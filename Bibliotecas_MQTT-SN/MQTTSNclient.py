...
import MQTTSN as MQTTSN
import MQTTSNinternal as MQTTSNinternal
...
class Callback:
  ...
  def connectionLost(self, cause):
    print("Conexao padrao perdida!", cause)
    self.events.append("disconnected")

  def messageArrived(self, topicName, payload, qos, retained, msgid):
    print("A publicacao padrao chegou!", topicName, payload, qos, retained, msgid)
    return True

  def deliveryComplete(self, msgid):
    print("Entrega padrao completa")
...
class Client:
  ...
  def __init__(self, clientid, host="localhost", port=10000):
...
def publish(topic, payload, retained=False, port=10000, host="localhost"):
  publish = MQTTSN.Publishes()
  publish.Flags.QoS = 3
  publish.Flags.Retain = retained
...
if __name__ == "__main__":
	...
	"""
  mclient = Client("myclientid", host="225.1.1.1", port=1883)
  mclient.registerCallback(Callback())
  mclient.start()
  
  publish("long topic name", "qos 0 start", port=1884)
  ...
  mclient.stop()
	...
	aclient = Client("linh", port=10000)
	aclient.registerCallback(Callback())
	aclient.connect()
	...
	aclient.publish("topic1", "aaaa", qos=1)
